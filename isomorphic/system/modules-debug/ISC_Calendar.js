...
/*
  ...
  Version v9.1p_2014-06-22/LGPL Development Only (2014-06-22)
...
if (window.isc && isc.version != "v9.1p_2014-06-22/LGPL Development Only") {
    isc.logWarn("SmartClient module version mismatch detected: This application is loading the core module from "
        + "SmartClient version '" + isc.version + "' and additional modules from 'v9.1p_2014-06-22/LGPL Development Only'. Mixing resources from different "
        ...
        if (!result) {
            switch (this.timelineGranularity) {
                case "day":
                    ...
                    // if each cell is a day, return "date", unless there are snapGaps within each cell
...
//
// @param event       (CalendarEvent) The event object to refresh in the current view
// @param styleName   (CSSStyleName) The new CSS style to apply to the windows for this event
// @visibility calendar
//<
...
createEventObject : function (sourceEvent, start, end, lane, name, description) {
...
getLaneFromPoint : function (x, y, view) {
    view = view || this.getSelectedView();
    if (!view.hasLanes || !view.hasLanes()) return null;
...
gridProps: {
    getPrintHTML : function (printProperties, callback) {
        ...
        output.append("<TABLE cellspacing='0' WIDTH=", totalWidth, " style='position: absolute; top:", bodyVOffset, "px; border: 1px solid grey;'>");
        ...
        output.append("<TABLE WIDTH=", totalWidth, " role='presentation' border='' class='", this.baseStyle, "' ",
            "style='borderSpacing:0px; position: absolute; top:", bodyVOffset,
            "px; z-index: -1;' ",
        ...
        for (var i=0; i<data.length; i++) {
            output.append(rowStart, heightAttr, this.getRowHeight(data[i]), gt);
        ...
        for (var i=0; i<events.length; i++) {
            ...
            output.append(event.getPrintHTML(printProperties, callback));
        }
            ...
            for (var i = numRows-1; i >= 0; i--) {
                ...
                for (var ii = startCol; ii < endCol; ii++) {
                    ...
                    HTML[HTML.length] = "style='margin: 0px; padding: 0px; " +
                        "width:" + fieldWidths[entry.masterIndex] + "px; " +
                        "border-width: 0px 1px 1px 0px;' "
                    ;
                    HTML[HTML.length] = ">";
    ...
    eventDragTargetDefaults: {
        ...
        dragRepositionMove : function () {
            ...
            var offsetX = (gr.getOffsetX() - props._startOffsetX),
                ...
                snapTop = isTL ? rowTop : Math.max(0, Math.min(props._maxTop, rowTop + snapY)),
        ...
        dragResizeMove : function () {
            ...
            if (isTimeline) {
                ...
                bottom = Math.min(gr.getScrollHeight(), bottom);
                height = bottom - top;
                width = right - left;
...
isc.DaySchedule.addProperties({
    ...
    rebuildFields : function () {
        ...
        if (cal.showDayLanes && cal.lanes && !this.isWeekView()) {
            ...
            var d = cal.chosenDate.duplicate(),
                scaffolding = isc.DaySchedule._getEventScaffolding(cal.getMinutesPerRow(this), d, this, cal),
            ...
            this.data = isc.DaySchedule._getEventScaffolding(cal.getMinutesPerRow(this), null, this, cal);
        }
    ...
    getCellDate : function (rowNum, colNum) {
        ...
        var fieldDate = this.body.fields[colNum].date,
            ...
            rowTime = record ? isc.Date.getLogicalTimeOnly(record.time) : null
        ;
    ...
    hasLanes : function () {
        return this.isDayView() && this.creator.showDayLanes;
    },
    ...
    getBaseStyle : function (record, rowNum, colNum) {
        ...
        if (currRowTime) {
            ...
            // need to set the time components of start and and comparison dates to the same
            // as workdayStart and workdayEnd
            wStart.setHours(parsedStart.getHours(), parsedStart.getMinutes(), parsedStart.getSeconds(), 0);
            wEnd.setHours(parsedEnd.getHours(), parsedEnd.getMinutes(), parsedEnd.getSeconds(), 0);
...
isc.EventWindow.addProperties({
    ...
    setEvent : function (event) {
        this.event = event;
        var styleName = event.eventWindowStyle;
    ...
    getPrintHTML : function (printProperties, callback) {
        ...
        if (isTimeline) {
            ...
            left = gridBody.getPageLeft() + this.getLeft();
            left += grid.isWeekView() ? 6 : 8;
        }
...
isc.TimelineView.addProperties({
    ...
    dragSelectCanvasDefaults: {
        ...
        resizeNow : function (props) {
            ...
            if (p.top == null) {
                p.top = view.getRowTop(view.getLaneIndex(p.lane.name));
            }
            if (p.height == null) {
                p.height = view.getLaneHeight(p.lane[cal.laneNameField]);
            }
    ...
    cellMouseDown : function (record, rowNum, colNum) {
        ...
        var canvas = this.getDragSelectCanvas(),
            endDate = this.getDateFromPoint(offsetX + cal.eventSnapGap, null, null, true),
            lane = record
        ;

        // don't allow selection if the day is a weekend and weekends are disabled
        if (cal.disableWeekends && !cal.dateIsWorkday(startDate, lane.name)) {
            return true;
        }

        var p = { lane: lane, startDate: startDate, endDate: endDate, top: null, height: null };
        canvas.resizeNow(p);
    ...
    cellOver : function (record, rowNum, colNum) {
        if (this._mouseDown) {
            var canvas = this.getDragSelectCanvas(),
                ...
                endDate = this.creator.addSnapGapsToDate(mouseDate, this, 1)
            ;
    ...
    cellMouseUp : function (record, rowNum, colNum) {
        ...
        var lane = props.lane,
            newEvent = cal.createEventObject(null, startDate, endDate, lane && lane.name)
        ;
        cal.showNewEventDialog(newEvent);
        this.clearSelection()
    },
    ...
    mouseMove : function () {
        if (this._mouseDown) {
            // call cellOver() to update the drag-selection canvas
            this.cellOver();
        }
    },
    ...
    */
    ...
    getDateLeftOffset : function (date, useNextSnapGap) {
        ...
        var localDate = date.duplicate(),
            cal = this.creator,
            visibleStartDate = cal.getVisibleStartDate(this),
            visibleEndDate = cal.getVisibleEndDate(this)
        ;
        ...
        for (var i=0; i<len; i++) {
            ...
            var startMillis = field.date.getTime(),
                endDate = new Date(startMillis + (this._getMinsInACell() * 60000)),
                endMillis = endDate.getTime(),
                startMins = Math.floor(field.date.getTime() / 60000),
                endMins = Math.floor(endDate.getTime() / 60000)
            ;
            if (mins < endMins) {
                if (mins >= startMins) {
...
isc.DaySchedule.addClassProperties({
    ...
    _getEventScaffolding : function (minsPerRow, startDate, view, calendar) {
        ...
        for (var i=0; i<rowCount; i++) {
            var time = isc.Date.getLogicalTimeOnly(date.duplicate());
